...
import { Button, Space, Row, Col, Layout, Card, Divider, Spin } from "antd";
...
import DupaPerProjectPreview from "./DupaPerProjectPreview";

const { Content } = Layout;

const DupaPerProjectDetails = ({ perProjectId, perDupaId }) => {
  ...
  const [perProj, setPerProjId] = useState(true);

  const { id } = perProjectId;
  ...
  const [laborFormData, setLaborFormData] = useState({
    id: "",
    dupa_content_per_project_id: perProjectId,
    labor_id: "",
    no_of_person: "",
    no_of_hour: "",
    group: "",
  ...
  const dupaLink = "/dupa-per-project";
  const dupaContentLink = "/dupa-content-per-project/" + perProjectId;
  const laborLink = "/dupa-labor-per-project";
  const equipmentLink = "/dupa-equipment-per-project";
  const materialLink = "/dupa-material-per-project";
  ...
  const dupaLaborLink = "/dupa-labor-per-project";
  const dupaEquipmentLink = "/dupa-equipment-per-project";
  const dupaMaterialLink = "/dupa-material-per-project";

  const dupaEquipmentAreaLink =
    "/dupa-content-per-project/equipment-area/" + perProjectId;
  const dupaEquipmentMinorToolLink =
    "/dupa-content-per-project/minor-tool-percentage/";
  const dupaAreaMaterialLink =
    "/dupa-content-per-project/material-area/" + perProjectId;
  const dupaConsumableMatLink =
    "/dupa-content-per-project/consumable-percentage/" + perProjectId;
  ...
  // UPDATE DUPA
  const updateDupa = async (_id) => {
    api
      .get(dupaLink + "/" + perProjectId)
      .then(function (response) {
        setDupaFormData({
          ...dupaFormData,
          id: response.data.id,
  ...
  // UPDATE DUPA LABOR
  const updateDupaLabor = async (_id) => {
    api
      .get(dupaLaborLink)
      .then(function (response) {
        const data = response.data.filter((item) => item.id === _id);
        setLaborFormData({
          ...laborFormData,
          id: data[0].id,
          labor_id: data[0].labor_id,
          dupa_content_per_project_id: data[0].dupa_content_per_project_id,
  ...
  // DELETE DUPA LABOR
  const deleteDupaLabor = (_id) => {
    Swal.fire(DeleteSwalConfig).then((result) => {
      if (result.isConfirmed) {
        api
          .delete(dupaLaborLink + "/" + _id)
          .then((response) => {
            Toast.fire({
              icon: "success",
  ...
  const updateDupaEquipment = async (_id) => {
    api
  ...
  const deleteDupaEquipment = (_id) => {
    Swal.fire(DeleteSwalConfig).then((result) => {
      if (result.isConfirmed) {
        api
          .delete(dupaEquipmentLink + "/" + _id)
          .then((response) => {
            Toast.fire({
              icon: "success",
              title: response.data.status,
  ...
  const updateDupaMaterial = async (_id) => {
    api
  ...
  const deleteDupaMaterial = (_id) => {
    Swal.fire(DeleteSwalConfig).then((result) => {
      if (result.isConfirmed) {
        api
          .delete(dupaMaterialLink + "/" + _id)
          .then((response) => {
            Toast.fire({
              icon: "success",
              title: response.data.status,
  ...
  const deleteEquipmentMinorTool = (_) => {
    Swal.fire(DeleteSwalConfig).then((result) => {
      if (result.isConfirmed) {
        api
          .delete(dupaEquipmentMinorToolLink + perProjectId)
          .then((response) => {
            Toast.fire({
              icon: "success",
              title: response.data.status,
  ...
  const onSubmit = async () => {
    ...
    if (selectedContent === "Labor") {
      ...
      dupaContentLink = dupaEquipmentMinorToolLink + perProjectId;
    ...
    console.log("form labor submit", dupaContentForm);
    await csrfCookie();
    api
  ...
  const fetchDupaContent = async () => {
    api
      ...
      .then(function (response) {
        ...
        console.log("content link", response.data);
        ...
        <Space size="small">
          {record.equipment_area ? (
            ...
            <Button
              ...
              type="link"
              icon={<DeleteOutlined />}
              onClick={() => deleteEquipmentMinorTool()}
            ></Button>
          ) : (
            <Space>
              <Button
  ...
  const updatedDataSource = [
    ...
    ...(dupaContent?.dupa_content?.dupa_equipment_per_project || []),
  ...
  const updatedDataSourceMaterial = [
    ...
    ...(dupaContent?.dupa_content.dupa_material_per_project || []),
              ...
              <div>
                ...
                <div className="responsive-table dupa-table-container">
                  <TableContentComponent
                    ...
                    dataSource={
                      dupaContent?.dupa_content.dupa_labor_per_project
                    }
                ...
                {/* <Row className="dupa-total-direct-cost">
                  <Col span={12}>
                    <p>
                      <strong>H.</strong> Overhead, Contingencies &
                      Miscellaneous (OCM)
                    </p>
                  </Col>
                  <Col span={7}>
                    <p style={{ textAlign: "center" }}>9% of G</p>
                  </Col>
                  <Col span={5} style={{ textAlign: "right" }}>
                    <p>{dupaContent?.h_ocm}</p>
                  </Col>
                </Row>
                ...
                <Row className="dupa-total-direct-cost">
                  <Col span={12}>
                    <p>
                      <strong>I.</strong> Contractor's Profit (CP)
                    </p>
                  </Col>
                  <Col span={7}>
                    <p style={{ textAlign: "center" }}>8% of G</p>
                  </Col>
                  <Col span={5} style={{ textAlign: "right" }}>
                    <p>{dupaContent?.i_contractors_profit}</p>
                  </Col>
                </Row>
                ...
                <Row className="dupa-total-direct-cost">
                  <Col span={12}>
                    <p>
                      <strong>J.</strong> Value Added Tax (VAT)
                    </p>
                  </Col>
                  <Col span={7}>
                    <p style={{ textAlign: "center" }}>12% of (G + H + I)</p>
                  </Col>
                  <Col span={5} style={{ textAlign: "right" }}>
                    <p>{dupaContent?.j_vat}</p>
                  </Col>
                </Row>
                ...
                <Row className="dupa-total-direct-cost">
                  <Col span={12}>
                    <p>
                      <strong>K.</strong> Total Unit Cost
                    </p>
                  </Col>
                  <Col span={7}>
                    <p style={{ textAlign: "center" }}>(G + H + I + J)</p>
                  </Col>
                  <Col span={5} style={{ textAlign: "right" }}>
                    <p>{dupaContent?.k_total_unit_cost}</p>
                  </Col>
                </Row> */}
              </div>
            )}
...
export default DupaPerProjectDetails;
